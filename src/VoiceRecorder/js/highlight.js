...
export function highlightWords(speech, text, callback) {
    var arr = text.trim().split(' ');
...
export function updateStutList(spanPos) {
    const stutDom = document.querySelector('.stutter');
    if(!stutDom || !stutDom.innerHTML) return;
 ...
 */
export function optimizeScript(text, stutList, callback) {
...
export function updateCarousel() {
    ...
    if (!html.length) {
        if (document.querySelector('.threshold')) {
